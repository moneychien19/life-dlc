...
## Application — Life DLC Chat UI

Life DLC's frontend is a **single-purpose product UI** (a personal RAG chat), not a marketing site. It adopts the Carbon token foundation above — `{colors}`, `{typography}`, `{spacing}`, `{rounded}` — but only a subset of components apply. **Reference-only (not built here):** `hero-card`, `cta-banner`, `customer-logo-tile`, `product-tab`, `newsletter-input`, `utility-bar`, `footer`. The chat components below are the ones that matter.

### Layout

- One centered column, **max-width ~720px**, on `{colors.canvas}`. Dense over airy — Carbon's 4px grid and `{spacing}` tokens throughout.
- **`chat-app-header`** pinned top: app title in `{typography.body-emphasis}`, 1px bottom `{colors.hairline}`, 48px tall. No logo chrome.
- **`chat-log`** scrolls in the middle.
- **`chat-composer`** pinned to the bottom.

### Messages

Two roles, distinguished by surface and alignment — never by shadow or radius.

- **`message-user`** (the question): `{colors.surface-1}` gray tile, `{rounded.none}`, right-aligned, max-width ~75%.
- **`message-assistant`** (the answer): `{colors.canvas}` with a **2px `{colors.primary}` left rule** (the one brand touch), full width, square. Inline citation markers `[1]`, `[2]` render in `{colors.primary}`.

### Sources

Each retrieved passage renders as a **`source-tile`** stacked under the assistant answer:
- `{colors.surface-1}` square tile, 1px `{colors.hairline}`.
- Header line: `[n]` index in `{colors.primary}` + document name in `{typography.body-emphasis}` + similarity score in `{typography.caption}` `{colors.ink-subtle}`.
- Passage excerpt in `{typography.body-sm}` `{colors.ink-muted}`.

### Composer & states

- **`chat-composer`**: `{colors.surface-1}` input, `{rounded.none}`; focus swaps the bottom hairline for a **2px `{colors.primary}` underline** (Carbon's signature focus). Paired **`chat-send`** = `button-primary` (blue, square). 48px min height on touch.
- **`empty-state`**: centered `{colors.ink-subtle}` prompt when there are no messages.
- **Loading**: a three-dot line inside an assistant bubble while awaiting `/chat`.
- **`error-banner`**: `{typography.body-sm}` in `{colors.semantic-error}` with a 2px error left rule, on a failed request.

### Applied rules

- IBM Blue stays **scarce**: citation markers, the send button, focused-input underline, and the assistant left-rule — nothing else.
- `{rounded.none}` on every bubble, tile, input, and button. No rounded chat bubbles.
- Hierarchy via surface change (`{colors.canvas}` ↔ `{colors.surface-1}`) + 1px hairlines. No drop shadows.
- IBM Plex Sans, body weight 400 with `letter-spacing: 0.16px` — keep the tracking.
